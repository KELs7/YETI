...
def buy_reward_token(contract: str, fee_cover_perc: float):
    #if nothing is passed to fee_cover_perc param we assume a default value
    ...
    #reserve %10 to cover for present and future transaction fees
...
@export
def distribute_rewards(contract: str, addresses: list, holder_min: float, distribute_min: float, fee_cover_perc: float):
    ...
    assert rewards_token_amount >= distribute_min, f'Not enough funds to distribute! Current amount is {rewards_token_amount}'
    ...
    for address in addresses:
        ...
        #check if holder has at least a certain minimum to qualify for reward
        if user_yeti_balance >= holder_min:
